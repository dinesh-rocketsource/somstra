    ...
    <form method="POST" action="{{ route("admin.customers.update", [$customer->id]) }}" enctype="multipart/form-data">
        ...
        <div class="body">
            ...
            <div class="mb-3">
                ...
                <div class="form-group">
                    <input type="email" id="email" name="email" class="{{ $errors->has('email') ? ' is-invalid' : '' }}" value="{{ old('email', $customer->email) }}" required>
                </div>
                @if($errors->has('email'))
            ...
            <div class="mb-3">
                ...
                <div class="form-group">
                    <input type="number" id="quiz_score" name="quiz_score" class="{{ $errors->has('quiz_score') ? ' is-invalid' : '' }}" value="{{ old('quiz_score', $customer->quiz_score) }}" required>
                </div>
                @if($errors->has('quiz_score'))
